...
// AFFICHAGE DES ROMANS ET NAVIGATION

function goToPage(id_roman, increment) {
    $.ajax({
        url: $('.div_link').attr('goToPage'),
        type: 'GET',
        data: {
            id_roman: id_roman,
            increment: increment,
            current_page: $('#page-roman').attr('current_page')
        }
    })
    .done(function(response) {
        $('#display-' + response['roman']).html(response['view']);
        manageNavigation();
    });
}
